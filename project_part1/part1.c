...
int main(void) {
    ...
    *(audio_ptr) = 0x0;                           // release the clear
    ...
    // NEW: Variables for the Noise Gate and Latch
    int saved_amplitude = 0;                      // Holds the last valid speech amplitude
    int NOISE_THRESHOLD = 15;                     // Minimum scaled value to trigger the HEX update
    ...
    while (1) {                                   // infinite loop
        ...
        if (lavailable > 0 && ravailable > 0) {   // if input samples are ready
            ...
            if (sample_count >= window_size) {     // if we've collected enough samples
                ...
                // CHANGED: Update LEDs as a real-time visualizer 
                *LED_ptr = max_amplitude >> 14;    

                // NEW: Scale amplitude down to remove ambient noise floor bits
                int scaled_amplitude = max_amplitude >> 12; 

                // NEW: Noise Gate and Latch logic
                // If the sound is louder than the background noise, update the HEX
                if (scaled_amplitude > NOISE_THRESHOLD) {
                    saved_amplitude = scaled_amplitude; // Save this new value
                    display_hex(saved_amplitude);       // Push it to the HEX display
                }
