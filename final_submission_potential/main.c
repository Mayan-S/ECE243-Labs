 ...
 *
 * Audio features:
 *   - Splash sound (splash.h) plays on any wave injection (mic, mouse)
 *     EXCEPT when rain mode is active.
 *   - Rain sound (drop_sample.h) loops continuously while rain mode is on.
 */

#include <stdlib.h>   /* rand() for rain mode random droplets */
#include "splash.h"       /* int splash_samples[SPLASH_N] — single splash sound */
#include "drop_sample.h"  /* int drop_sample[RAIN_N]       — looping rain sound */
...
int main(void) {
    ...
    *(pixel_ctrl_ptr + 1) = (int)&Buffer2;
    ...
    while (1) {
        ...
        /* 1a. Audio top-off — keep FIFO fed at the top of every frame */
        output_audio();
        ...
        *(key_ptr + 3) = 0xF;           /* clear edge capture */
         ...
         *        fixed amplitude (-128), loudness controls only the size of the splash. */
        if (mic_amp > MIC_THRESHOLD) {
            if (voice_follows_cursor) {
                /* Same rules as mouse: amplitude always -128, loudness → size.
