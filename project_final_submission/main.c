/*
    ...
    KEY1 toggles voice origin between center and mouse cursor position.
    KEY2 toggles rain mode (random droplets + looping rain audio).
...
#include <stdlib.h>       // rand() for rain mode random droplets 
#include "splash.h"       // single splash sound 
#include "drop_sample.h"  // looping rain sound 
...
int voice_follows_cursor = 0;    // 0 = voice at center, 1 = voice at mouse cursor

// Audio playback state
int playing_splash = 0;          // 1 = currently playing splash sound 
int splash_pos = 0;              // current position in splash_samples[]
int splash_shift = 0;            // volume: 0 = full, 1 = half, 2 = quarter
int rain_mode = 0;               // 1 = rain mode active 
int rain_pos = 0;                // current position in drop_sample[] (loops)
...
void trigger_splash(short int amplitude);
...
int main(void) {
    ...
    *(pixel_ctrl_ptr + 1) = (int)&Buffer2;
    ...
    while (1) {
        ...
        // 0. Audio top-off — keep FIFO fed at the top of every frame
        output_audio();
        ...
        // 2. Read KEY0/KEY1/KEY2 via edge capture register
        int pressed = *(key_ptr + 3);
        *(key_ptr + 3) = 0xF;           // clear edge capture

        if (pressed & 0x1) {            // KEY0 = reset
            clear_grids(grid_curr, grid_prev, grid_next);
            mouse_hold_frames = 0;
            playing_splash = 0;
            rain_mode = 0;
        ...
        if (pressed & 0x2) {            // KEY1 = toggle voice origin mode
            voice_follows_cursor = !voice_follows_cursor;
        }

        if (pressed & 0x4) {            // KEY2 = toggle rain mode
            rain_mode = !rain_mode;
            if (!rain_mode) {
                rain_pos = 0;            // reset rain audio position
            }
        }

        // 2b. Rain mode — inject random droplets across the grid
        if (rain_mode) {
            for (int d = 0; d < 20; d++) {
        ...
        /* 4. If loud enough, inject a wave.
              voice_follows_cursor == 0: inject at center, loudness controls amplitude and size.
              voice_follows_cursor == 1: inject at mouse cursor, same rules as mouse click.
        */
        if (mic_amp > MIC_THRESHOLD) {
            if (voice_follows_cursor) {
                /* Same rules as mouse: amplitude always -128
                   mic_amp < 100  --> size 0 (single pixel)
                   mic_amp < 200  --> size 1 (cross)
                   mic_amp >= 200 --> size 2 (3×3 with diagonals) 
                */
                int size;
                if (mic_amp >= 200)      size = 2;
        ...
        // 4b. Read mouse — accumulate movement for this frame
        {
            ...
            if (got_packet) {
                ...
                if (mouse_last_buttons & 0x1) {
                    if (is_moving) {
                        ...
                        /* Stationary + clicking: grow SIZE over time, amplitude stays -128.
                           ...
                           Grows every 10 frames. 
                        */
                        mouse_hold_frames++;
                        int size = mouse_hold_frames / 10;
                        if (size > 2) size = 2;
                        inject_source_sized(grid_curr, mouse_y, mouse_x, MOUSE_DROP_AMP, size);
                        // Only trigger splash on the first frame of a hold
                        if (mouse_hold_frames == 1) {
                            trigger_splash(MOUSE_DROP_AMP);
                        }
                    }
                } else {
                ...
                /* mouse is perfectly still (No packets this frame)
        ...
        // 4c. Audio top-off after input processing
        if (playing_splash || rain_mode) output_audio();

        // 5. Run wave physics — writes next state into grid_next
        wave_physics(grid_next, grid_curr, grid_prev, damping_factor);

        // 5a. Audio top-off after physics
        if (playing_splash || rain_mode) output_audio();
        ...
        // 6c. Final audio top-off before vsync wait
        if (playing_splash || rain_mode) output_audio();
...
/*  Audio engine
    Feeds the audio FIFO every time it is called.
    Priority: rain sound (looping) > splash sound (one-shot) > silence. */
...
void output_audio(void) {
    ...
    while (wslc > 0 && wsrc > 0) {
        ...
        } else if (playing_splash) {
            // One-shot splash sound with volume control
            int sample = splash_samples[splash_pos] >> splash_shift;
            *(audio_ptr + 2) = sample;   // left
            *(audio_ptr + 3) = sample;   // right

            splash_pos++;
            if (splash_pos >= SPLASH_N) {
                playing_splash = 0;      // done
            }
        } else {
            // Silence — prevents static buzzing
            *(audio_ptr + 2) = 0;
            *(audio_ptr + 3) = 0;
        }
...
/* Trigger splash sound
    ...
    Starts (or restarts) the splash sound effect.
    Does nothing if rain mode is active (rain audio takes priority).
    Volume is based on the injected wave amplitude:
    |amp| >= 100 --> full volume
    |amp| >= 50  --> half volume
    else         --> quarter volume
*/
...
void trigger_splash(short int amplitude) {
    if (rain_mode) return;   // splash sound disabled during rain

    playing_splash = 1;
    splash_pos = 0;          // rewind, interruptible

    if (amplitude <= -100 || amplitude >= 100) {
        splash_shift = 0;    // full volume
    } else if (amplitude <= -50 || amplitude >= 50) {
        splash_shift = 1;    // half volume
    } else {
        splash_shift = 2;    // quarter volume
    }
}
...
/* Microphone input
   ...
   There are about 133 samples (60 fps / 8 kHz) per frame.
*/
...
int read_mic_amplitude(void) {
    ...
    while (1) {
        ...
        // Codec data is in bits [23:8]
        // Shift down to get useful range
...
/* Inject a source at (cy, cx).
   Amplitude is multiplied by AMP_SCALE so the grid stores values in the full short int range, preventing truncation death.
...
/*
  ...
  Amplitude is always the same (full dark color). Only the pattern size varies.
...
/*
 ...
 Drains the FIFO after init to prevent stale bytes from desyncing the state machine.
*/
void mouse_init(void) {
    unsigned char byte;
    int timeout;
    ...
    // Drain anything left over and start clean
    while (ps2_read_byte(&byte));
}

/* 
   Try to read a complete 3-byte mouse packet (non-blocking).
   Uses a static state machine so partial packets are accumulated across multiple calls from the main loop.
...
int mouse_read(int *dx, int *dy, int *buttons) {
    ...
    while (ps2_read_byte(&byte)) {
        ...
        // Mouse sends 0xAA then 0x00 on power-up.
        // When this sequence is seen, re-send enable data reporting.
        if (byte == 0xAA) {
            saw_aa = 1;
