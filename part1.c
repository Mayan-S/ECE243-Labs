 ...
 *
 * Audio samples are 24-bit signed. The absolute peak is scaled to a
 * 0–255 wave amplitude value and shown on the HEX displays.
 */
    ...
    while (1) {
        ...
        if ((fifospace & 0x000000FF) > 0) {
            ...
            if (sample_count >= WINDOW_SIZE) {
                /* Map mic peak to wave amplitude 0–255 */
                int wave_amp = (peak * 255) / MIC_PRACTICAL_MAX;
                if (wave_amp > 255) wave_amp = 255;  /* clamp if louder than expected */

                /* Show scaled wave_amp on HEX (00–FF on HEX1-HEX0) */
                display_hex((unsigned int) wave_amp);
                peak = 0;
                sample_count = 0;
            }
        }
